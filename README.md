...
- **AccessからCSVをエクスポートする際は、文字列フィールドにクォーテーション（`""`）を付けて出力してください。**
  Access の「テキストのエクスポート ウィザード」では「テキスト区切り記号」に `"` を指定することでクォート付き出力になります。クォートなしでエクスポートすると、`to_notion` でのID→名前変換が正しく機能しない場合があります。
- **AccessへCSVをインポートする際は、以下の設定を確認してください。**
  - 「先頭行をフィールド名として使う」にチェックを入れる
  - 「テキスト区切り文字」に `"` を指定する
